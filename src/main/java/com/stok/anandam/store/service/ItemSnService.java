...
public class ItemSnService {
    ...
    public List<ItemSerialNumberResponse> getSnData(String type, String search, String docId, String user,
        ...
        if ("MASUK".equalsIgnoreCase(type)) {
            ...
            docCol = "p.doc_no";
            userCol = "p.par_name";
            sql.append("SELECT p.doc_date AS tanggal, p.doc_no AS doc_id, p.par_name AS user, m.name AS item_name, sn.sn ")
            ...
            docCol = "s.doc_no";
            userCol = "s.par_name";
            sql.append("SELECT s.doc_date AS tanggal, s.doc_no AS doc_id, s.par_name AS user, m.name AS item_name, sn.sn ")
                    .append("FROM dbtitemsn sn ")
                    .append("LEFT JOIN dbmitem m ON sn.ite_id = m.id ")
        ...
        if (search != null && !search.isBlank()) {
            ...
            sql.append("AND (sn.sn LIKE ? OR ").append(docCol).append(" LIKE ? OR ").append(userCol).append(" LIKE ? OR m.name LIKE ?) ");
            params.add(term);
            params.add(term);
            params.add(term);
            params.add(term);
        ...
        if (sortBy != null && ALLOWED_SORT_FIELDS.contains(sortBy)) {
            switch (sortBy) {
                case "docId": orderColumn = docCol; break;
                case "user": orderColumn = userCol; break;
                case "itemName": orderColumn = "m.name"; break;
                case "sn": orderColumn = "sn.sn"; break;
                case "tanggal":
                default: orderColumn = dateColumn; break;
            }
        }
        ...
        return jdbcTemplate.query(sql.toString(), params.toArray(), (rs, rowNum) -> ItemSerialNumberResponse.builder()
                .tanggal(rs.getDate("tanggal") != null ? rs.getDate("tanggal").toLocalDate() : null)
